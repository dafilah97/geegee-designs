...
import { Plus, UploadCloud, Video, Package } from "lucide-react";
...
export default function AdminProducts() {
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Upload Image
      const imageUrl = await uploadFile(imageFile, 'product-media');
    ...
    <div className="max-w-6xl mx-auto">
      ...
      {isAdding && (
        <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 mb-8">
          <h2 className="text-xl font-serif mb-6">Create New Product</h2>
          <form onSubmit={handleAddProduct} className="space-y-6">
            ...
            <div className="flex justify-end gap-4 pt-4 border-t border-gray-100">
              <button type="button" onClick={() => setIsAdding(false)} className="px-6 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">Cancel</button>
              <button disabled={isSubmitting} type="submit" className="px-6 py-2 bg-black text-white rounded-lg text-sm font-medium hover:bg-gray-800 transition-colors disabled:opacity-50">
                {isSubmitting ? (uploading ? "Uploading..." : "Saving...") : "Save Product"}
              </button>
            </div>
          </form>
        </div>
